...
function minifyStyles() {
  return gulp.src(`${paths.distCss}/styles.css`)
    .pipe(cleanCss())
...
function minifyCssReset() {
  return gulp.src(`${paths.distCss}/reset.css`)
    .pipe(cleanCss())
    .pipe(rename({ suffix: '.min' }))
    .pipe(gulp.dest(paths.distCss));
}
...
function serve(done) {
  ...
  gulp.watch(`${paths.scss}/**/*.scss`, gulp.series(styles, cssReset, minifyStyles, minifyCssReset));
...
// Build task
const build = gulp.series(clean, gulp.parallel(html, styles, cssReset, minifyStyles, minifyCssReset, vendors, scripts, images));
